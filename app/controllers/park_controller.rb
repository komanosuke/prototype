...
class ParkController < ApplicationController
    ...
    def products
        ...
        p @products
        # @bench = @benches.find(params[:bench_id])
        # @camera = Camera.find(park_id: @park.id) #
        ...
        if area_code != ''
            ...
            @wind = @response['forecasts'][0]['detail']['wind']
            chance_of_rain = @response['forecasts'][0]['chanceOfRain']
            @chance_of_rain = []
            chance_of_rain.each_value do |c|
                @chance_of_rain.push(c)
            end
        end
    end
